...
public class TableGenerator {
     ...
     * under a root node hierarchy with PBR materials instead of returning a single cube.
     */
    public static SceneObject generateTable(String rootId, String rootName, MaterialManager matMgr) {
        ...
        // 1. Root Table Container Node
        SceneObject tableRoot = new SceneObject(rootId, rootName, "TABLE", null, null);

        // 2. Tabletop Surface (Wide, flat, thin height)
        SceneObject top = new SceneObject(rootId + "_top", "Tabletop Surface", "STRUCTURE",
                PrimitiveGenerator.createCube(1.8f, 0.08f, 1.0f), topMat);
        top.getTransform().setPosition(0f, 0.76f, 0f);
        tableRoot.addChild(top);

        // 3. Under-Top Support Beams (2 long structural beams running lengthwise)
        ...
        lBeam.getTransform().setPosition(0f, 0.71f, -0.35f);
        tableRoot.addChild(lBeam);

        SceneObject rBeam = new SceneObject(rootId + "_beam_r", "Support Beam Right", "STRUCTURE",
                PrimitiveGenerator.createCube(1.6f, 0.06f, 0.06f), legMat);
        rBeam.getTransform().setPosition(0f, 0.71f, 0.35f);
        tableRoot.addChild(rBeam);

        // 4. Supporting Legs (4 separate cylinders reaching down to the ground)
        float lx = 0.75f, lz = 0.38f;
        ...
        for (int i = 0; i < 4; i++) {
            SceneObject leg = new SceneObject(rootId + "_leg_" + i, "Table Leg " + (i + 1), "STRUCTURE",
                    PrimitiveGenerator.createCylinder(0.04f, 0.72f, 8), legMat);
